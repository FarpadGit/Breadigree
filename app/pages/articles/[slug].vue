        ...
        <div class="grid grid-cols-1 md:grid-cols-[3fr_1fr] gap-8">
            <div class="flex flex-col gap-6">
                <div class="relative flex items-center overflow-hidden max-h-96 hover:max-h-screen transition-[max-height] duration-500 ease-out rounded-2xl group">
                    <nuxt-img :src="currentArticle?.featureImage" width="2500" class="bg-[color-mix(in_srgb,var(--color-background),var(--color-secondary)_40%)] brightness-50 group-hover:brightness-100 transition-[filter] duration-500" />
                    <h1 
...
<script setup lang="ts">
    ...
    const articles = (await useFetch<articleType[]>("/api/articles")).data.value;
    const currentArticle = articles?.find(post => post.slug === articleSlug);
    if (!currentArticle) await navigateTo("/");
</script>
